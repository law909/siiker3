<div class="term_lista">
	...
	{if (count($kosarlista.data))}
			...
			<form action="index.php" method="post">
				<div class="termek_rovidle_container">
					...
					<div class="termek_rovidle_img">
						<a href="{$_termek.bigimageuri}"><img class="rovidle" src="{$_termek.smallimageuri}" title="{$_termek.imagealt}" /></a>&nbsp;
				 	</div>
					 {else}
				...
				<br />
				<div class="ar_n szoveg_bold">{$text->ar->nettoertek}{number_format($_termek.nettoertekhuf,0,',',' ')}&nbsp;{$_termek.valutanem}</div>
				<div class="ar_b szoveg_bold">{$text->ar->bruttoertek}{number_format($_termek.bruttoertekhuf,0,',',' ')}&nbsp;{$_termek.valutanem}</div>
		...
		<a href="{$kosarlista.adatbekerouri}" class="button"><span>{$spectext->kosar->gomb->ok2}</span></a>
		{else}
